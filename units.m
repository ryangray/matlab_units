...
%  units base dimension % Prints the base unit values for dimension given.
...
%  units system sysname  % Set units system base (si, english, cgs, etc.)
%  units system list  % List available units system bases
...
%  units install      % Add units folder to the path
%
...
%
% See also: unitsSymbols, unitsPlot, unit2str, unitval
...
elseif ischar(varargin{1})
    ...
    switch lower(varargin{1})
        ...
        case 'install'
            ...
            base = fileparts(mfilename('fullpath'));
            addpath(base,'-END')
            fprintf('The pathtool should open with the units and folder added at the end.\n');
            fprintf('You should save the new path for future use.\n');
            fprintf('You can use "units system xxxx" to change the units system base if you like.\n');
            fprintf('You can use "units system list" to see a list of valid ones.\n');
        ...
        case 'system'
            ...
            if length(varargin) > 1
                ...
                % Get units folder path

                si = fileparts(which(mfilename));

                if strcmpi(varargin{2},'list')
                    ...
                    if ~strcmpi(varargin{2},'si')
                        ...
                        rmpath(oldpathstr);       % Remove the old one
        ...
        case 'class'
            ...
            if nargin == 1
                ...
                dbl = fileparts(which(mfilename));
                oldpathstr = fileparts(which('unit_MAKE'));
